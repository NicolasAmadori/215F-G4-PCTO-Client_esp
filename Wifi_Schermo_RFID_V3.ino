...
const uint16_t server_port = 60101;
const char * server_IP = "172.20.10.4";
const uint16_t local_port = 60101;
...
WiFiServer wifiServer(local_port);
...
void WifiConnection(){
  ...
  Serial.println("Connected to Wifi. Assigned IP: " + WiFi.localIP());
...
String ReadTag(boolean isAuthenticated = true){
  ...
  if(isAuthenticated){
    ...
    Serial.println("Waiting for CIE...");  
  }
...
void loop(void) {
  ...
  wifiServer.begin();
  String messaggioRicevuto = ReceiveMessage();
  wifiServer.stop();
  
  Serial.println("Message reveived: " + messaggioRicevuto);
}
